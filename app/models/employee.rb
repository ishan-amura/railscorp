class Employee < ActiveRecord::Base
  ...
  VALID_EMAIL_REGEX = /\A[a-z][a-z0-9]+@[[a-z\-]|[^\.\W\_]]+[\.a-z][a-z]+\.[[a-z]|[^\.\W\_]]{2,6}\z/i
	# Regex Break down
	# \A[a-z] = makes sure first character is a albhabet 
	# [[a-z0-9]+@  = selects all characters except space until @
	# [[-z\-]|[^\.\W\_]]+  = selects subdomain 
	# [\.a-z][a-z]+   = selects main domain
	# \.[[a-z]|[^\.\W\_]]{2,6} = selects domain extension (.com, .org, .in, .co.in, etc)
	
	validates :name, :email, :designation, presence: true, length: { maximum: 50 }
	
	validates :name, format: { with: /\A[a-zA-Z\s]+\z/, 
														 message: "only supports letters and spaces" }
	...
	def self.search args
		if args.length == 2
			if !args[:name].nil? and !args[:email].nil?
				Employee.joins(:address).where(name:args[:name],email:args[:email]).pluck(:name,:email,:city,:state,:locality)
			end
		elsif  args.length == 1
			if !args[:name].nil?
				Employee.joins(:address).where(name:args[:name]).pluck(:name,:email,:city,:state,:locality)
			elsif !args[:email].nil?
				Employee.joins(:address).where(email:args[:email]).pluck(:name,:email,:city,:state,:locality)	
			end
		end
	end
